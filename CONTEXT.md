...
## Session log (2026-07-06) — Phase C: webhook event history, rebooking safety, captain-input decision (Claude Code)

**Goal:** answer a batch of 11 questions Omri raised about the FareHarbor webhook pipeline — which notes/comments actually reach us, whether the webhook→Bookings→Tours flow should be an event log, whether to capture more fields, how rebooking behaves, and how the captain should report end-of-tour status. Ran a live capture experiment (temporary), then implemented the confirmed findings as "Phase C." Spec: `docs/superpowers/specs/2026-07-05-webhook-event-log-review.md`. Plan: `docs/superpowers/plans/2026-07-06-webhook-phase-c.md`.

**Empirical findings (live webhook capture, 2026-07-06):** booking notes, custom-field edits, check-in/no-show, and rebooking all fire a `booking.updated` webhook and are captured correctly end-to-end (verified: check-in payload → `Checked In` count in Bookings matched exactly). **Activity/availability-level notes (private or public) do NOT fire any webhook** — confirmed by direct test, not assumed. Slot-level crew reassignment also isn't reliably re-fired. Rebooking: the old booking flips to status `rebooked`; a **new booking with a new PK** is created; they're linked by **UUID** (old carries `rebooked_to`, new carries `rebooked_from` — both top-level string fields, confirmed from a real captured payload, not from FareHarbor docs). Notes/custom fields carry over to the new booking. FareHarbor fires 2+ near-duplicate webhooks per single logical change (all captured, none lost — the existing 25s lock serializes them).

**Captain end-of-tour input — decision:** captains get **no dashboard, no login**. Their entire contribution is checking guests in on FareHarbor, which now flows fully automatically into Bookings/Tours. The two things webhooks can never carry (both activity-level: crew/captain assignment changes, last-minute headcount not tied to a booking) are covered by the **ops team** during their existing approval step, not by the captain. A future **captain checklist tool**, wired into the ops dashboard, is the eventual real interface — out of scope now. This kills any design that tries to make captains trigger webhooks via booking-level hacks.

**Implementation (6 tasks, each deployed independently, task-by-task with editor-run tests between each):**
1. **Raw tab is now append-only** (`writeRaw`, `Webhook.js`) — one row per webhook event instead of upsert-by-UUID, so Raw is a full change-history log, not a "latest snapshot." Deliberate deviation from the spec's "separate Events tab" recommendation — Omri asked to fold it into the existing Raw tab instead, one fewer tab to manage. All three existing Raw readers (`buildRawAvailabilityLookup`, QBO's affiliate-shortname cache, `rebuildBookingsFromRaw`) already resolve last-write-wins by UUID, so none needed changes.
2. **New Bookings columns:** `Display ID`, `Created At`, `Rebooked From`, `Rebooked To` (`Config.js` COLS.BOOKINGS, `Webhook.js` `extractBookingData`) — field names confirmed against a real rebooking payload, not guessed.
3. **Rebook guard (`applyRebookGuard_`, `Webhook.js`):** on any booking with `rebooked_from` set, looks up the predecessor by UUID in Bookings; if it's still `booked` (its own `rebooked` webhook was lost or late), marks it `rebooked` and recomputes that old availability's tour too — closing the one real double-count gap the architecture review flagged.
4. **Nightly `rebuildTours()` safety net** added to `morningRun` (`Helpers.js`), before the QBO block, wrapped in its own try/catch so it can never block QBO posting — dry-run verified live: rebuilt 754 tours from 1505 bookings in 11s.
5. **More Bookings columns**, added only after confirming presence in ~60 real recent payloads first (test function, not assumption): `Contact Phone` (40/60 — not every guest has one, still real), `Language`, `Amount Paid (MXN)`, `Receipt Subtotal`, `Receipt Taxes`, `Source Type` (all 60/60).
6. **Removed all temporary research code** used to run the capture experiment: `TEST_CAPTURE` Script Property, `logWebhookTestCapture_`, `readWebhookTestLog`, `testDumpWebhookLog`/`testDumpMarkedWebhooks`/`testVerifyCheckin`, the `?action=webhook_test_log` API endpoint, and the `Webhook Test Log` sheet tab. Confirmed via repo-wide grep that no references remain. The 4 Phase C regression tests (`testRawAppendOnly`, `testExtractNewFields`, `testRebookGuard`, `testCheckPhase5FieldPresence`) are independent of the removed capture code and were kept.

**Backfill:** existing Bookings rows don't retroactively get the new columns populated — `rebuildBookingsFromRaw_Start`/`_Continue` (already-existing batched, resumable rebuild) can backfill from Raw's history if needed; optional, not run as part of this session.

**Deployments:** apps-script pinned deployment progressed @162 → @163 (Raw append-only) → @164 (new identity/rebook columns) → @165 (rebook guard) → @166 (contact/payment/source columns) → @167 (cleanup). Each step tested live in the Apps Script editor before deploying.

**Out of scope (future, per spec):** mid-year pruning of Raw if it grows too large (annual archive already bounds it; not needed yet); Drive overflow storage for >49k-char payloads (none observed — largest seen ~44k chars); the pre-existing QBO sandbox TaxCode error and a pre-existing duplicate-booking finding from 2026-06-22 (both noticed during this session's `morningRun` dry-run, unrelated to Phase C, not fixed here).

## Session log (2026-07-07) — Insumos inventory (check-out/check-in) + Gasolina quick inputs (Claude Code)

**Goal:** ultra-fast phone inputs for the ops coordinator. Insumos (bebidas/hielo/snacks) as a per-tour **check-out/check-in** flow prefilled from a per-tour-product "receta"; gasolina (gasolinera + lancha) and insumo purchases from one global button. Spec: `docs/superpowers/specs/2026-07-06-insumos-gasolina-inputs-design.md`; plan: `docs/superpowers/plans/2026-07-06-insumos-gasolina-inputs.md`. Executed subagent-driven (implementer + reviewer per task).

**Data model:** 3 new tabs — **`Insumos Catálogo`** (Item/Unidad/Categoría/Precio/Mínimo/Máximo/Activo; owner-edited; Precio auto-refreshes to unit cost when a purchase with Cantidad is logged), **`Recetas Tour`** (Tour/Item/Por Persona/Cantidad Fija — per FareHarbor product name, exact match; each line is per-pax OR fixed), **`Insumos Tours`** (the salida/regreso records: Consumido = salida − regreso, Costo = consumido × catalog price at check-in time). The existing **`Insumos`** tab gained a `Cantidad` column and acts as the purchase/restock log (KPI cost math untouched — still reads Cost (MXN)). **Stock is computed, never stored:** purchases − consumed, cached 5 min (`insumos_stock_v1` CacheService key), invalidated on checkout/checkin/purchase. Mínimo drives "stock bajo" chips; Máximo reserved for a future shopping-list view.

**Backend (`Inventory.js`, new):** `insumosPrefill` (receta × pax, merged with saved state; state machine none/salida/complete), `insumosCheckout` (idempotent — re-checkout replaces the tour's rows), `insumosCheckin` (fills regreso/consumido/costo; regreso defaults to salida), `insumosStock`, `logInsumoPurchase` (wraps `logInsumo` + catalog price refresh). Read actions `insumos_prefill`/`insumos_stock` in API.js doGet; write actions `insumos_checkout`/`insumos_checkin`/`insumo_purchase` as key-gated text/plain doPost branches (same contract as save_report/ops_approve_tour). Deployed **@168**; routing curl-verified (Unauthorized JSON for bad key). Editor test `testInsumosFlow` passed end-to-end live (prefill qty 8 for 2/persona × 4 pax → checkout → checkin 3 returned → consumido 5, costo $50, stock −5, low=true); `cleanupInsumosTestRows` run after. Note: bulk tab creation hit the recurring "Service Spreadsheets timed out" on this large doc — `setupInventorySheetsStepwise()` (one tab per run + flush) is the reliable creator.

**Frontend (`ops.html`):** the previously-dead "Gastos / Inventario" per-tour button now opens the Inventario panel — thumb-sized ± steppers prefilled from the receta, "Confirmar salida", then the same panel pre-fills regreso = salida ("todo regresó" is zero taps) → "Confirmar regreso"; 📦✓ / 📦✓✓ chip next to the button; "+ Agregar item" from the catalog during salida; "stock bajo" chips; item names HTML-escaped (sheet-sourced). Global **⛽ Gastos** button above the list → Gasolinera (bidones + costo, GET `gas_station`), Lancha (boat datalist from the day's tours' recursos, GET `gas_boat` — these two are doGet actions via `parseInputs`, NOT doPost), Compra (POST `insumo_purchase`, updates catalog price). Checkin POST deliberately omits `tour_name` (not in the contract). Coordinator identity = existing `getApprover()` device-name seam.

**Deployments:** apps-script @168; dashboard commit `fa4acf0` (+177 lines, ops.html only).

**Out of scope (per spec):** shopping-list/restock view (Máximo stored for it), catalog/recetas admin UI (sheet-edited), merma/stock-adjustment entry (negative-Cantidad purchase works), real login.
